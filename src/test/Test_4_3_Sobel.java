 ...
 */
public class Test_4_3_Sobel {
    public static void main(String[] args) throws IOException {
        String fileName = "orange_flower";
        Image image = new Image("tests/test4_sobel", fileName, "png");

        // Sobel
        int sobelThres = 110;    // Min and max pixel intensity (both ways)
        image = Sobel.applySobel(image, sobelThres);
        image.saveImage("tests/test4_sobel/test4.3", fileName + "_sobel-" + sobelThres);

        // Segmentation
        int segThres = 25;      // Max pixel intensity
        ...
        image.saveImage("tests/test4_sobel/test4.3", fileName + "_sobel-" + sobelThres + "_segmentize_thres-" + segThres);
    }
}
